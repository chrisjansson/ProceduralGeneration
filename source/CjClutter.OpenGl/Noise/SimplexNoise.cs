/*
 * 
 * C# port of simplex noise algorithm originally from
...
namespace CjClutter.OpenGl.Noise
{
    ...
    public class SimplexNoise
    {
        ...
        private readonly double _f2 = 0.5 * (Math.Sqrt(3.0) - 1.0);
        private readonly double _g2 = (3.0 - Math.Sqrt(3.0)) / 6.0;
        private const double F3 = 1.0/3.0;
        private const double G3 = 1.0/6.0;
        private readonly double _f4 = (Math.Sqrt(5.0) - 1.0) / 4.0;
        private readonly double _g4 = (5.0 - Math.Sqrt(5.0)) / 20.0;
        ...
        public double Noise(double xin, double yin)
        {
            ...
            // c = (3-sqrt(3))/6
            var x1 = x0 - i1 + _g2; // Offsets for middle corner in (x,y) unskewed coords
            var y1 = y0 - j1 + _g2;
            var x2 = x0 - 1.0 + 2.0 * _g2; // Offsets for last corner in (x,y) unskewed coords
            var y2 = y0 - 1.0 + 2.0 * _g2;
        ...
        public double Noise(double x, double y, double z, double w)
        {
            ...
            var s = (x + y + z + w) * _f4; // Factor for 4D skewing
            var i = Fastfloor(x + s);
            var j = Fastfloor(y + s);
            var k = Fastfloor(z + s);
            var l = Fastfloor(w + s);
            var t = (i + j + k + l) * _g4; // Factor for 4D unskewing
            ...
            var x1 = x0 - i1 + _g4; // Offsets for second corner in (x,y,z,w) coords
            var y1 = y0 - j1 + _g4;
            var z1 = z0 - k1 + _g4;
            var w1 = w0 - l1 + _g4;
            var x2 = x0 - i2 + 2.0 * _g4; // Offsets for third corner in (x,y,z,w) coords
            var y2 = y0 - j2 + 2.0 * _g4;
            var z2 = z0 - k2 + 2.0 * _g4;
            var w2 = w0 - l2 + 2.0 * _g4;
            var x3 = x0 - i3 + 3.0 * _g4; // Offsets for fourth corner in (x,y,z,w) coords
            var y3 = y0 - j3 + 3.0 * _g4;
            var z3 = z0 - k3 + 3.0 * _g4;
            var w3 = w0 - l3 + 3.0 * _g4;
            var x4 = x0 - 1.0 + 4.0 * _g4; // Offsets for last corner in (x,y,z,w) coords
            var y4 = y0 - 1.0 + 4.0 * _g4;
            var z4 = z0 - 1.0 + 4.0 * _g4;
            var w4 = w0 - 1.0 + 4.0 * _g4;
